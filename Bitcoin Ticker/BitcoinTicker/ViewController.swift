...
class ViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    // numberOfComponents, pickerView(numberOfRowsInComponent) are required to conform to the protocol
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        finalURL = "\(baseURL)\(currencyArray[row])" // API
    ...
    func updatePriceData(json : JSON) {
        if let priceResult = json["ask"].double {
            self.bitcoinPriceLabel.text = "\(self.currencySelected)\(priceResult)"
        } else {
